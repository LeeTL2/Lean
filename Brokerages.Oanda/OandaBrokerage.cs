...
namespace QuantConnect.Brokerages.Oanda
{
    ...
    public class OandaBrokerage : Brokerage, IDataQueueHandler
    {
        ...
        {
            Initialize(orderProvider, securityProvider, aggregator, accessToken, accountId, agent);
        }

        /// <summary>
        /// Gets the Oanda environment from the OANDA_ENVIRONMENT environment variable.
        /// Practice is used as the safe default when no environment is specified.
        /// </summary>
        private static Environment GetEnvironment()
        {
            var value = System.Environment.GetEnvironmentVariable("OANDA_ENVIRONMENT")
                        ?.Trim()
                        .ToLowerInvariant();

            return value switch
            {
                "live" => Environment.Trade,
                "trade" => Environment.Trade,
                "practice" => Environment.Practice,
                null or "" => Environment.Practice,
                _ => throw new InvalidOperationException(
                    $"Invalid OANDA_ENVIRONMENT '{value}'. Expected 'practice' or 'live'.")
            };
        }
        ...
        public bool IsValidHistoryRequest(
            Symbol symbol,
        ...
        private IEnumerable<BaseData> GetHistoryImpl(HistoryRequest request)
        {
            ...
            while (startDateTime <= request.EndTimeUtc)
            {
                ...
                startDateTime = quoteBars[quoteBars.Count - 1]
                    .Time
        ...
        /// </summary>
        /// <param name="job">Job we're subscribing to</param>
        ...
        public IEnumerable<TradeBar> DownloadTradeBars(
            Symbol symbol,
            DateTime startTimeUtc,
            DateTime endTimeUtc,
            Resolution resolution,
        ...
        /// <param name="requestedTimeZone">The requested timezone for the data</param>
        /// <returns>The list of bars</returns>
        public IEnumerable<QuoteBar> DownloadQuoteBars(
            Symbol symbol,
            DateTime startTimeUtc,
            DateTime endTimeUtc,
            Resolution resolution,
        ...
        /// <summary>
        /// Initializes the Oanda REST API.
        /// </summary>
        /// <param name="orderProvider">The order provider.</param>
        /// <param name="securityProvider">The holdings provider.</param>
        /// <param name="aggregator">Consolidate ticks.</param>
        /// <param name="accessToken">The Oanda access token.</param>
        /// <param name="accountId">The account identifier.</param>
        /// <param name="agent">The Oanda agent string.</param>
